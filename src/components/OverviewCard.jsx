...
export const OverviewCard = ( {user, audienceType, audience, network, today, isUp} ) => {
  return (
    <article className='bg-Light-Grayish-Blue w-[326px] h-[216px] rounded-[5px] overflow-hidden text-center dark:bg-Dark-Desaturated-Blue hover:brightness-95 cursor-pointer hover:dark:brightness-125'>
        ...
        <div className='flex items-center place-content-center gap-2'>
            ...
            <p className='text-xs text-Dark-Grayish-Blue font-bold dark:text-Desaturatedated-Blue '>{user}</p>
        </div> 
        <p className='text-[56px] font-bold text-Very-Dark-Blue dark:text-White'>{audience}</p>
        <p className='uppercase tracking-[5px] text-Dark-Grayish-Blue text-xs mb-6 dark:text-Desaturatedated-Blue '>{audienceType}</p>
...
export const OverviewTodayCard = ( {network, statsType, stats, porcentage, isUp} ) => {
  return (
    <article className='bg-Light-Grayish-Blue w-[326px] h-[125px] mb-4 mx-auto rounded-[5px] p-[27px] cursor-pointer dark:bg-Dark-Desaturated-Blue hover:brightness-95 cursor-pointer hover:dark:brightness-125'>
        <div className='flex items-center place-content-between'>
            <p className='dark:text-Desaturatedated-Blue font-bold'>{statsType}</p>
            <img src={networkLogos[network]} alt={`logo ${network}`} />
        </div>
        <div className='flex justify-between'>
            <p className='text-Very-Dark-Blue dark:text-White font-bold text-[42px]'>{stats}</p>
            <div className='flex items-center place-content-between gap-1'>
                <img src={ isUp ? iconUp : iconDown } alt="icon arrow" />
